...
class OscController extends Controller
{
	...
	public function updateDadosGerais(Request $request, $id)
    {
    	...
    	foreach($json as $key => $value){
			...
			if($json[$key]->cd_situacao_imovel_osc != $cd_situacao_imovel){
                ...
                $id_tabela = $json[$key]->id_osc;
                $tx_dado_anterior = $json[$key]->cd_situacao_imovel_osc;
			...
			if($json[$key]->tx_nome_responsavel_legal != $nome_responsavel_legal){
                ...
                $id_tabela = $json[$key]->id_osc;
                $tx_dado_anterior = $json[$key]->tx_nome_responsavel_legal;
                $tx_dado_posterior = $nome_responsavel_legal;
				$resultDaoLog = $this->log->insertLogDadosGerais($tx_nome_campo, $id_usuario, $id_tabela, $tx_dado_anterior, $tx_dado_posterior);
            }
			...
			if(strlen($ano_cadastro_cnpj) > 4){
				if($json[$key]->dt_ano_cadastro_cnpj != $ano_cadastro_cnpj){
                    ...
                    $id_tabela = $json[$key]->id_osc;
                    $tx_dado_anterior = $json[$key]->dt_ano_cadastro_cnpj;
                    $tx_dado_posterior = $ano_cadastro_cnpj;
    				$resultDaoLog = $this->log->insertLogDadosGerais($tx_nome_campo, $id_usuario, $id_tabela, $tx_dado_anterior, $tx_dado_posterior);
                }
			...
			if(strlen($dt_fundacao) > 4){
				if($json[$key]->dt_fundacao_osc != $dt_fundacao){
                    ...
                    $id_tabela = $json[$key]->id_osc;
                    $tx_dado_anterior = $json[$key]->dt_fundacao_osc;
                    $tx_dado_posterior = $dt_fundacao;
    				$resultDaoLog = $this->log->insertLogDadosGerais($tx_nome_campo, $id_usuario, $id_tabela, $tx_dado_anterior, $tx_dado_posterior);
                }
			...
			if($json[$key]->tx_resumo_osc != $resumo){
                ...
                $id_tabela = $json[$key]->id_osc;
                $tx_dado_anterior = $json[$key]->tx_resumo_osc;
                $tx_dado_posterior = $resumo;
                $resultDaoLog = $this->log->insertLogDadosGerais($tx_nome_campo, $id_usuario, $id_tabela, $tx_dado_anterior, $tx_dado_posterior);
            }
    ...
    public function updateApelido(Request $request, $id)
    {
    	...
    	foreach($json as $key => $value){
    		...
    		if($json[$key]->tx_apelido_osc != $apelido){
                ...
                $id_tabela = $json[$key]->id_osc;
                $tx_dado_anterior = $json[$key]->tx_apelido_osc;
                $tx_dado_posterior = $apelido;
	...
	public function setContatos(Request $request, $id)
	{
		...
		if($telefone != null){
            ...
            $id_tabela = $json[$key]->id_osc;
            $tx_dado_anterior = $json[$key]->tx_telefone;
		...
		if($email != null){
            ...
            $id_tabela = $json[$key]->id_osc;
            $tx_dado_anterior = $json[$key]->tx_email;
		...
		if($site != null){
            ...
            $id_tabela = $json[$key]->id_osc;
            $tx_dado_anterior = $json[$key]->tx_site;
	...
	public function setParticipacaoSocialConselho(Request $request, $id_osc)
	{
		...
		foreach($conselho_req as $key_req => $value_req){
			...
			if(isset($value_req['representante'])){
				$representante = $value_req['representante'];
			}
			...
			foreach ($conselho_osc as $key_conselho_osc => $value_conselho_osc) {
				if($value_conselho_osc->cd_conselho == $cd_conselho){
					...
					if($value_conselho_osc->cd_tipo_participacao != $cd_tipo_participacao || $value_conselho_osc->tx_periodicidade_reuniao != $tx_periodicidade_reuniao || $value_conselho_osc->dt_data_inicio_conselho != $dt_data_inicio_conselho || $value_conselho_osc->dt_data_fim_conselho != $dt_data_fim_conselho){
						...
						$reresentante_osc = DB::select($query, [$id_conselho])->tx_nome_representante_conselho;

						$flag_insert_representante = true;
						foreach ($reresentante_osc as $key_reresentante_osc => $value_reresentante_osc) {
							$tx_nome_representante_conselho = $value_reresentante_osc->tx_nome_representante_conselho;
							if($representante){
								foreach ($representante as $key_representante => $value_representante) {
									if($tx_nome_representante_conselho == $value_representante['tx_nome_representante_conselho']){
										$flag_update_representante = false;
									}
								}
							}
		...
		foreach($recursos_req as $key_req => $value_req){
			...
			foreach ($recursos_db as $key_db => $value_db) {
				if($value_db->cd_fonte_recursos_osc == $cd_fonte_recursos_osc && $value_db->dt_ano_recursos_osc == $dt_ano_recursos_osc){
					...
					if($value_db->nr_valor_recursos_osc != $nr_valor_recursos_osc){
